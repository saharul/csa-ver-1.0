...
def EditService():
    ...
    try:
        ...
        defmile = int(df1.loc[index, 'Mileage'])
        defnxtmile = int(df1.loc[index, 'Nxt_Mileage'])
        defnxtdate = df1.loc[index, 'Nxt_Date']
        defamount = "{:.2f}".format(df1.loc[index, 'Amount'])
        ...
        amount = inputFloat("\nEnter Service Amount, default [%s]: " %defamount, defamount)
...
def DispCursMenu():
    menu_list=["ADD A NEW SERVICE", "DISPLAY SERVICE & PARTS", "DELETE A SERVICE",\
            "EDIT SERVICE", "SEARCH FOR SERVICES","GET TOTAL NO OF SERVICES",\
            "VIEW ALL SERVICES", "VIEW NEXT SERVICE DATE", "SPARE PART MENU"]

    menu = SelectionMenu(menu_list,"Car Services Application", subtitle="MAIN MENU")

    # # A SelectionMenu constructs a menu from a list of strings
    # parts_menu = SelectionMenu(["item1", "item2", "item3"])

    # # A SubmenuItem lets you add a menu (the selection_menu above, for example)
    # # as a submenu of another menu
    # submenu_item = SubmenuItem("Spare Part Menu", parts_menu, menu)
...
def DisplayChoice():
    ...
    while choice != 9:
        ...
        # display Service information and its Parts
            ...
            clear_screen()
            PrintBlankRow(1)
            print_red_on_cyan("There are " + str(GetTotal()-1) + " number of services!")
            PrintBlankRow(1)
            input('\nPress <ENTER> to continue')
...
def DispCursPartMenu():
    menu_list=["ADD A NEW SERVICE PART", "EDIT A SERVICE PART", "DELETE A SERVICE PART",\
            "GO BACK TO MAIN MENU", "VIEW ALL SERVICE PARTS","SEARCH SERVICE PARTS"]

    menu = SelectionMenu(menu_list,"Car Services Application", subtitle="SPARE PARTS MENU")
...
def DisplayPartsMenu():
    ...
    print_blue_on_green("{0:*^36s}".format("PARTS MENU"))
    print("{0:>1s} {1:s}".format(" ","CHOOSE AN OPERATION:"))
    
    menu = []
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","1", "- ADD A NEW SERVICE PART"))
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","2", "- EDIT A SERVICE PART"))
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","3", "- DELETE A SERVICE PART"))
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","4", "- GO BACK TO MAIN MENU"))
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","5", "- VIEW ALL SERVICE PARTS"))
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","6", "- SEARCH SERVICE PARTS"))
    menu.append("{0:>1s} {1:0>2s} {2:s}".format(" ","9", "- EXIT PROGRAM"))
...
def DisplayPartsChoice():
    ...
    while (choice != 6):
        ...
        #GET USER CHOICE
        #choice = input("{0:>1s} ".format("Select an operation (1,2..9): "))

        choice = DispCursPartMenu()
